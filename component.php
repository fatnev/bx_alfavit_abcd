...
if($this->StartResultCache(false, array($arrFilter_not_abcd, $letter_id, $USER->GetGroups())))
{
	...
	else
	{
		...
		if($arParams["SHOW_RUS"] && !$arParams["GROUP_RUS"])
			...
			$arResult['letters']=array_merge(Array(
				GetMessage("A"),GetMessage("B"),GetMessage("V"),GetMessage("G"),
				GetMessage("D"),GetMessage("E"),GetMessage("J"),GetMessage("Z"),
				GetMessage("I"),GetMessage("K"),GetMessage("L"),GetMessage("M"),
				GetMessage("N"),GetMessage("O"),GetMessage("P"),GetMessage("R"),
				GetMessage("S"),GetMessage("T"),GetMessage("U"),GetMessage("F"),
				GetMessage("H"),GetMessage("C"),GetMessage("C1"),GetMessage("S1"),
				GetMessage("S2"),GetMessage("Y"),GetMessage("E1"),GetMessage("U1"),
				GetMessage("A1")),$arResult['letters']);
...
elseif($letter=="rus")
{
    ${$FILTER_NAME}[">NAME"] = GetMessage("A");
    ${$FILTER_NAME}["<=NAME"] = GetMessage("A111");
}
elseif($letter=="eng")
{
    ${$FILTER_NAME}[">NAME"] = "A";
